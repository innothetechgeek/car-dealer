@extends('layouts.master')
@section('body')
<div class = "row" >
    <div class="col-md-4 offset-md-4" id = "signup-form">
        <form method="POST" action="{{ url('car/edit-car/'.$car->id) }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <div class="fake-input">
                    <input type="text" name = 'name' class="form-control  @error('name') is-invalid @enderror"" value = "{{$car->name}}" id="" placeholder="Name">
                    <img src="{{url('assets/images/form_icon.png')}}" width=25 />
                </div>
                @error('name')
                  <label  class="error"><strong>{{ $message }}</strong></label>
                @enderror
            </div>
            <div class="form-group">
                <div class="fake-input">
                    <input type="number" name = 'price' class="form-control  @error('price') is-invalid @enderror" id="" value = "{{$car->price}}" placeholder="Price">
                    <img src="{{url('assets/images/form_icon.png')}}" width=25 />
                </div>
                @error('price')
                    <label  class="error"><strong>{{ $message }}</strong></label>
                @enderror
            </div>
            <div class="form-group">
                <div class="card mb-4" style="width: 18rem;">
                    <div class="card-body" style = "padding-bottom: 1rem !important;">
                        <?php 
                            $exists = Storage::disk('public_images')->exists($car->image);
                            $image_path = $exists ? "assets/images/".$car->image : 'assets/images/uploads/vehicle-placeholder.png';
                        ?>
                        <img  height = '165' class="card-img-top" src="{{ asset($image_path) }}" alt="Card image cap">
                    </div>
                </div>
                <input type="file" name="image" class="form-control{{ $errors->has('file') ? ' is-invalid' : '' }}" >
                @error('image')
                    <label  class="error"><strong>{{ $message }}</strong></label>
                @enderror    
            </div>
            <input type="submit" value="Save" class="btn form-btn  bcg-dark btn-block">
        </form>
    </div>
</div>
@endsection